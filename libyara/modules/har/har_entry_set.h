...
const char *str __attribute__((unused));
  {
    ...
    if(node) {
      ...
      if(str) yr_set_string(str, module_object, path);
    }
    path[pathlen] = 0;
  }
  {
  ...
  }
  {
    ...
    if(node) {
      yr_set_float(json_real_value(node), module_object, path);
    }
    path[pathlen] = 0;
  }
  {
    ...
    }
    {
      ...
      json_array_foreach(node, index, value) {
        ...
        }
        {
          ...
          if(node) {
            yr_set_integer(json_integer_value(node), module_object, path);
          }
          path[pathlen] = 0;
        }
        {
